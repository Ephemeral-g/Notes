...
StringHash::StringHash(const long tableLen)
{
	InitCryptTable();
	m_tableLen = tableLen;
...
// 求取哈希值
unsigned long StringHash::HashString(const std::string &str, unsigned long nHashType)
{
...
// 检测一个字符串是否被hash过
unsigned long StringHash::Hashed(const std::string &str)
{
...
// hash一个字符串
bool StringHash::Hash(const std::string &str)
{
	const unsigned long HASH_OFFSET = 0, HASH_A = 1, HASH_B = 2;
	unsigned long nHash = HashString(str, HASH_OFFSET);
	unsigned long nHashA = HashString(str, HASH_A);
